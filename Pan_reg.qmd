...
$\widehat{FatalityRate_{it}} = \beta_0 + \beta_1 BeerTax_{it} + \beta_2Zi + u_{it}$
...
$FatalityRate_{i1988} = \beta_0 + \beta_1 BeerTax_{it} + \beta_2Zi + u_{i1988}$
$FatalityRate_{i1982} = \beta_0 + \beta_1 BeerTax_{it} + \beta_2Zi + u_{i1982}$

Suppose $E{(u_{it}}{/BeerTax_{it},Z_i})=0$
E(uit|BeerTaxit, Zi) = 0.
...
so 
$FatalityRate_{i1988} – FatalityRate_{i1982}=\\\beta_1(BeerTaxi1988 – BeerTaxi1982) + (u_{i1988} – u_{i1982})$

•	The new error term, $(u_{i1988} – u_{i1982})$, is uncorrelated with either $FatalityRate_{i1988}$ or $FatalityRate_{i1982}$
...
$Y_{CA,t} = \beta_0 + \beta_1X_{CA,t} + \beta_2Z_{CA} + u_{CA,t}\\
			  = (\beta_0 + \beta_2Z_{CA}) + \beta_1X_{CA,t}  + u_{CA,t}$
or
$Y_{CA,t} = \alpha + \beta_1X_{CA,t} + u_{CA,t}$


•	$\alpha_{CA} = \beta_0 + \beta_1Z_{CA}$  doesn’t change over time
...
Population regression for Texax (that is, $i = TX$):
$Y_{TX,t} = \beta_0 + \beta_1X_{TX,t} + \beta_2Z_{TX} + u_{TX,t}\\
			  = (\beta_0 + \beta_2Z_{TX}) + \beta_1X_{TX,t}  + u_{TX,t}$
or
...
or 
$Y_{it} = \alpha_{i} + \beta_1 X_{it}  + u_{it}, i =CA,TX,MA, T = 1,\dots,T$ 

## Regression equation for three states

![](fixed_effect.png)
In binary regressor form:
$Y_{it} = \beta_0 + \gamma CADCAi + \gamma TXDTXi + \beta_1X_{it} + u_{it}$
•	$DCA_{i} = 1$ if state is $CA, = 0$ otherwise
•	$DTX_{t} = 1$ if state is $TX, = 0$ otherwise
•	leave out $DMA_{i}$ (why?)
...
In this model, the OLS estimate of the parameter of interest $\beta_1$
  is equal to the estimate obtained using (10.2) — without the need to estimate $n-1$
...
where $T-1$ are binary variables ($B_1$ is ommited)
*The entity time fixed effect* is
...
2.    $(X_{i1}, X_{i2}, \dots, X_{i3}, u_{i1}, \dots, u_{iT})$ are i.i.d draw from their distributions. 
3.    Large outliers are unlikely, i.e., $(u_{it},x_{it})$ have nonzero finite fourth moments. 
4.    There is no perfect multicollinearity. 
When there are multiple regressors, $x_{it}$ is replaced with $X_{1,it}, X_{2,it}, \dots, X_{k,it}$. 
...
The most appropriate standard errors account for within-person serial correlation and are robust to heteroskedasticity. 
